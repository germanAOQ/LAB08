...
public class AlquilerItemsBean extends BasePageBean {
	...
	public long consultarMulta(Item it) {
		long multa = 0;
		try{
			multa= serviciosAlquiler.consultarMultaAlquiler(it.getId(),(java.sql.Date) new Date(System.currentTimeMillis()));
		}
		catch(ExcepcionServiciosAlquiler e){
            setErrorMessage(e);
        }
	...
	public void registrarAlquilerCliente(int id,int numDiasAlquilar){
        try{
            ...
            serviciosAlquiler.registrarAlquilerCliente((java.sql.Date) new Date(System.currentTimeMillis()),selectedCliente.getDocumento(),item,numDiasAlquilar);
        }catch(ExcepcionServiciosAlquiler e){
            setErrorMessage(e);
        }
    }
